...
public class Sudoku extends LatinSquare {
	...
	private boolean fillRemaining(Cell c) {
		...
		for(int n : c.listValidValues) {
			if(isValidValue(c.iRow, c.iCol, n)) {
				this.getPuzzle()[c.iRow][c.iCol] = n;
				if(fillRemaining(c.GetNextCell(c))) {
					return true;
				}
				this.getPuzzle()[c.iRow][c.iCol] = 0;
			}
		}
		return false;
	}
	...
	private void setCells() {
		for(int i = 0; i < iSize; i++) {
			for(int j = 0; j < iSize; j++) {
				Cell c = new Cell(i,j);
				c.setListValidValues(getAllValidCellValues(i,j));
	...
	private void showAvailableValues() {
		for(int i = 0; i < iSize; i++) { 
			for(int j = 0; j < iSize; j++) {
				Cell c = cells.get(Objects.hash(i, j));
				System.out.print("Cell- Row" + i + " Column" + j + " values= ");
				for(int n : c.getListValidValues()) {
					System.out.print(n + " ");
				}
				System.out.println("");
			}
	...
	private class Cell {
		...
		private int iCol;
		private int iRow;
		private ArrayList<Integer> listValidValues = new ArrayList<Integer>();
		...
		public void setListValidValues(HashSet<Integer> lstValidValues) {
			listValidValues = new ArrayList<Integer>(lstValidValues);
		}
		...
		public void ShuffleValidValues() {
			Collections.shuffle(listValidValues);
		}
...
//			if(c.iRow == iSize - 1 && c.iCol == iSize - 1) {
//				return null;
//			} else if(c.iCol == iSize - 1) {
//				return (Cell)cells.get(Objects.hash(0,iRow + 1));
//			} else {
//				return (Cell)cells.get(Objects.hash(c.iCol + 1, c.iRow));
//			}
		}	
	}
}